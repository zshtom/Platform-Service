...
class IndexController extends ActionController
{
    protected function render($row)
    {
        ...
        if (!$row instanceof RowGateway || !$row->active) {
            $title      = __('Solution request');
            $content    = __('The solution requested does not exist.');
            $markup     = '';
        } else {
            $content    = $row->content;
    ...
    public function indexAction()
    {
        ...
        if ($id) {
            ...
            $row = $this->getModel('solution')->find($name, 'name');
        } elseif ($slug) {
            $row = $this->getModel('solution')->find($name, 'slug');
        }
